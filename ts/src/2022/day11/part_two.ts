    ...
    for (let i = 0; i < 10000; i++) {
        for (const monkey of monkeys) {
            for (const item of monkey.items) {
                ...
                if (item_clone >= global_divisor) item_clone %= global_divisor;
                if (item_clone >= Number.MAX_SAFE_INTEGER) throw 'too big';
...
if (require.main === module) {
    (async () => {
        ...
        console.log(`Result: ${await benchmark(() => equation_two(input))}`); //32059801242 ~55.51ms
    })();
}
