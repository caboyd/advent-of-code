...
function isFull(cards: string) {
    ...
    if (count['J'] && isTwoPair(cards)) return true;
...
function isX(cards: string, amount: number) {
    ...
    for (const [key, num] of Object.entries(count)) {
        if (key == 'J' && num != 5) continue;
        if (count['J']) {
            if (num + count['J'] >= amount) return true;
        } else if (num >= amount) return true;
    }
    return false;
}
...
if (require.main === module) {
    (async () => {
        ...
        console.log(`Result: ${await benchmark(() => equation_two(input))}`); //251421071 ~12.7ms
    })();
}
