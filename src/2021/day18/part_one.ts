...
function PairToString(s: Pair): string {
    let result = '[';
...
function createSnailfish(s: string): Snailfish {
...
function reduce(r: Snailfish): void {
    do {
        r.reduced = false;
        r.reduced = explode(r);
...
function magnitude(r: Pair | number | undefined): number {
    if (!r) return 0;
...
export const equation_one = (input: string): number => {
    ...
    for (const line of lines) {
       ...
       // console.log(PairToString(left_snail));
        reduce(left_snail);
       // console.log(PairToString(left_snail));
    }
